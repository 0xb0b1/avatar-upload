import { expect } from 'vitest'
import { fireEvent, screen, render } from '@testing-library/react'
import { Crop } from './Crop'
import { ImageContext } from '../../contexts/ImageContext'

describe('Crop image', () => {
  it('should render slider with correct DOM structure', () => {
    const { asFragment } = render(<Crop />)

    expect(asFragment().firstChild).toMatchSnapshot()
  })

  it('should render canvas correctly', () => {
    render(<Crop />)

    expect(screen.getByTestId('uploaded-avatar')).toBeInTheDocument()
  })

  it('should show an image that the user selected', () => {
    const image =
      'data:image/png;base64,'
    render(
      <ImageContext.Provider value={{ image }}>
        <Crop />
      </ImageContext.Provider>,
    )

    expect(screen.getByTestId('uploaded-avatar')).toBeInTheDocument()
  })

  it('should save the image and send user to initial state', () => {
    const handleImageState = vi.fn()
    render(
      <ImageContext.Provider value={{ handleImageState }}>
        <Crop />
      </ImageContext.Provider>,
    )

    const saveImageButton = screen.getByTestId(
      'save-image-btn',
    ) as HTMLButtonElement

    fireEvent.click(saveImageButton)

    expect(handleImageState).toHaveBeenCalled()
  })

  it('should close crop section when clicking the X', () => {
    const handleCancelEdit = vi.fn()
    render(
      <ImageContext.Provider value={{ handleCancelEdit }}>
        <Crop />
      </ImageContext.Provider>,
    )

    const cancelEditButton = screen.getByTestId(
      'close-image-edit',
    ) as HTMLButtonElement

    fireEvent.click(cancelEditButton)

    expect(handleCancelEdit).toHaveBeenCalled()
  })

  // it('should be able to change image position with mouse', () => {
  //   const handleImagePosition = vi.fn()
  //   render(
  //     <ImageContext.Provider value={{ handleImagePosition }}>
  //       <Crop />
  //     </ImageContext.Provider>,
  //   )

  //   const uploadedAvatar = screen.getByTestId('uploaded-avatar')

  //   fireEvent.change(uploadedAvatar, { x: 0.5, y: 0.5 })

  // console.log('uploadedAvatar', uploadedAvatar)

  // expect(uploadedAvatar.value).toMatchObject({ x: 0.5, y: 0.5 })
  // })
})
