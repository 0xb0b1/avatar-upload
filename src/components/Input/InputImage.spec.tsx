...
describe('Image Uploader', () => {
  ...
  it('should have an input of type file', () => {
    ...
    const uploadAvatarInput = screen.getByTestId(
      'upload-avatar',
    ) as HTMLInputElement

    expect(uploadAvatarInput).toHaveAttribute('type', 'file')
  })

  it('should be able to add an image by clicking on the input', () => {
    const handleFileChange = vi.fn()
    const file = new File(['image'], 'image.png', { type: 'image/png' })

    render(
      <ImageContext.Provider value={{ handleFileChange }}>
        <InputImage />
  ...
  it('should show an image if the context state is available', () => {
    const image =
